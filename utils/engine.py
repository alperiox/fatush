...
from utils.script import read_file, split_to_line_chunks, clean_chunks, line_chunks_to_documents
from utils.config import add_config, remove_config, load_config, create_config, CONFIG_PATH
...
def fetch_documents_from_repo(source_lang: t.Optional[str], target_lang: t.Optional[str],
    ...
    subprocess.run(["mv", os.path.join(path, "fastapi", "docs"), os.path.join(path, "docs")])
...
def process_source_files(paths: t.List[t.Tuple[Path, Path]]) -> t.Tuple[t.List[Document], t.List[dict]]:
    ...
    for translated_filepath, source_filepath in track(paths, description="[cyan]Processing the files"):
    ...
    all_source_documents = [doc for doc in all_source_documents if doc.page_content != ""]

    return all_source_documents, all_source_chunks

def construct_vector_store(documents: t.List[Document], embedding_model: HuggingFaceEmbeddings, vectorstore_path: t.Optional[str] = ".") -> None:
...
def load_vector_store(embedding: HuggingFaceEmbeddings, vectorstore_path: t.Optional[str] = ".") -> FAISS:
